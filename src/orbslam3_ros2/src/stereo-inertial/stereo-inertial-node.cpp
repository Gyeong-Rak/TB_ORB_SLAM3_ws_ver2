    ...
    SLAM_(SLAM)
{
    ...
    this->declare_parameter("world_frame", "map"); // 세계 좌표계 프레임
    this->declare_parameter("odom_frame", "odom"); // 로봇의 이동을 기준으로 한 프레임
    this->declare_parameter("body_frame", "zed_camera_link"); // ZED 카메라의 본체 프레임
    this->declare_parameter("body_optical_frame", "zed_imu_link"); // ZED IMU 프레임
    this->declare_parameter("camera_optical_frame", "zed_left_camera_optical_frame"); // ZED 카메라의 왼쪽 광학 프레임
